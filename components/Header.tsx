...
export function Header(props: { isOpen: boolean; humbergurBtnFunc: () => void; hidden?: boolean }) {
    return (
        <>
            <header
                hidden={props.hidden}
                className="fixed top-0 w-full py-2 md:py-4 bg-green-800 z-50 dark:bg-stone-950"
            >
                <div className="relative flex pl-4 mx-auto gap-3 md:gap-6">
                    <button
                        onClick={() => props.humbergurBtnFunc()}
                        type="button"
                        className="z-10 space-y-1 md:space-y-2"
                    >
                        <div
                            className={
                                props.isOpen
            ...
            <div hidden={props.hidden} className="h-11 md:h-16"></div>
        </>
    );
}
